...
class WPPPOB_Admin {
    ...
    public function __construct() {
        ...
        add_action('admin_init', [$this, 'process_category_form']);
    ...
    /**
 * Memproses dan menyimpan data dari form Kelola Kategori Tampilan.
 */
public function process_category_form() {
    if (!isset($_POST['cat_name']) || !isset($_POST['_wpnonce'])) {
        return;
    }
    ...
    // Cek apakah kategori memiliki anak
    if ($category_id > 0) {
        $child_count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(id) FROM {$table_name} WHERE parent_id = %d", $category_id));
        if ($child_count > 0) {
            // Jika punya anak, jangan simpan produk, kosongkan saja
            $data['assigned_products'] = '[]';
        } else {
            // Jika tidak punya anak, proses produk yang dipilih
         ...
         // Kategori baru pasti belum punya anak, jadi proses produk
        $assigned_products = isset($_POST['assigned_products']) && is_array($_POST['assigned_products']) ? array_map('intval', $_POST['assigned_products']) : [];
        $data['assigned_products'] = json_encode($assigned_products);
    }

    // Tentukan format data untuk query
    $format = ['%s', '%d', '%s', '%s', '%s', '%s'];
