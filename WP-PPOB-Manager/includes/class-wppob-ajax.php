...
class WPPPOB_Ajax {
    ...
    public function update_product_order_in_category() {
        // Nonce harus sesuai dengan yang di-enqueue di class-wppob-admin.php
        ...
        if (false === $result) {
            wp_send_json_error(['message' => 'Gagal memperbarui database.']);
        } else {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_wppob_update_product_order_in_category', [$this, 'update_product_order_in_category']);
    }
    ...
    public function ajax_get_category_content() {
        ...
        if ($parent_category && !empty($parent_category->assigned_products)) {
            ...
            if (!empty($product_ids)) {
               $products = wc_get_products([
    'include' => $product_ids,
    'limit'   => -1, // Mengambil semua produk yang dipilih
    'orderby' => 'post__in', // Mengurutkan berdasarkan urutan yang disimpan
]);
                foreach ($products as $product) {
                    $data['products'][] = [
                        'id' => $product->get_id(),
